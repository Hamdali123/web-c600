...
async function syncSerialNumbers() {
    ...
    try {
        ...
        // We will send all commands in one go to save time and VTY lines
        // But for 200 ONUs, telnet-client might timeout if we send 200 commands.
        // Let's do it in batches of 20.
        const batchSize = 20;
        let updatedCount = 0;
        ...
        for (let i = 0; i < onus.length; i += batchSize) {
            ...
            require('fs').writeFileSync('debug_sync.txt', output);
            ...
            for (const line of lines) {
                ...
                if (trimmed.startsWith('ONU interface:')) {
                    currentInterface = trimmed.substring(trimmed.indexOf(':') + 1).trim(); // gpon_onu-1/2/14:8
                    ...
                    // Use description as name if name is empty/masked, but ensure we don't set '********'
                    if (match && !currentName && !match[1].includes('********')) {
                        currentName = match[1].trim();
                    }
                } else if (trimmed.startsWith('Serial number:') && currentInterface) {
                    const sn = trimmed.split(':')[1].trim(); // ZTEGCD13E23F
                    
                    // Parse back to DB format to update
                    ...
                    if (portMatch.length === 2) {
                        ...
                        const updateData: any = { sn_mac: sn };
            ...
            console.log(`[Sync SN] Batch ${i/batchSize + 1} selesai. Total updated: ${updatedCount}`);
        }
        
        console.log(`[Sync SN] SELESAI! Berhasil menarik ${updatedCount} Serial Number asli.`);
    } catch (e: any) {
