...
export async function POST(
  ...
  try {
    ...
    const { adminState, description } = body;
    ...
    if (creds.vendor === 'zte') {
       const { updateUplinkPortCommand } = await import('@/lib/vendors/zte-c600');
       const script = updateUplinkPortCommand(decodedPortName, adminState, description);
       await executeOltCommand(creds, script, { failOnError: true });
       return NextResponse.json({ success: true, message: `Uplink port ${decodedPortName} updated successfully.` });
    }
    
    return NextResponse.json({ error: 'Unsupported vendor for this action' }, { status: 400 });
  } catch (error: any) {
    console.error(`Update Uplink port failed:`, error);
