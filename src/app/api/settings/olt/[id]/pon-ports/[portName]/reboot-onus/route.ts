...
export async function POST(
  ...
  try {
    ...
    if (creds.vendor === 'zte') {
       ...
       // Normalize both formats so gpon_olt-1/2/1 (parser/UI) and gpon-olt_1/2/1 (DB) always match:
       // strip the prefix entirely, then compare against both persisted forms.
       const portKey = decodedPortName.replace(/^gpon[-_]olt[-_]/i, '');
       const dbFormats = [`gpon-olt_${portKey}`, `gpon_olt-${portKey}`];

       const onus = await prisma.oNUConfigured.findMany({
          where: { olt_id: oltId, pon_port: { in: dbFormats } }
       });
       ...
       for (const onu of onus) {
          if (!onu.pon_port) continue;
          const interfacePort = onu.pon_port.replace('gpon-olt_', 'gpon_onu-').replace('gpon-onu_', 'gpon_onu-');
