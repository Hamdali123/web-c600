...
export async function POST(
  ...
  try {
    ...
    if (creds.vendor === 'zte') {
      ...
      const uplinkPorts = ZteC600.parseUplinkPorts(output);
      return NextResponse.json({ success: true, ports: uplinkPorts });
    }
    
    return NextResponse.json({ success: true, ports: [] });
  } catch (error: any) {
    console.error("Refresh Uplink Ports failed:", error);
