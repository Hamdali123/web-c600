...
export default function OltVlansPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchVlans = async () => {
    setError('');
    try {
      const res = await fetch(`/api/settings/olt/${id}/vlans`, { cache: 'no-store' });
              ...
              {loading && vlans.length === 0 ? (
                ...
                vlans.map((vlan, idx) => (
                  <tr key={idx} style={{ borderBottom: '1px solid #eee' }}>
                    ...
                    <td className="text-center" style={{ verticalAlign: 'middle' }}><input type="checkbox" disabled /></td>
                    <td className="text-center" style={{ verticalAlign: 'middle' }}>
                      <Link href={`/onu/configured?vlan=${vlan.id || vlan.vlan_id}&olt=${id}`} style={{ color: '#337ab7', textDecoration: 'underline' }}>{vlan.onu_count || 0}</Link>
                    </td>
                    <td className="text-center" style={{ verticalAlign: 'middle' }}>
