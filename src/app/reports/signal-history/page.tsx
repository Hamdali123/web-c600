...
export default function SignalHistoryPage() {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      try {
        const res = await fetch('/api/reports/signal-history');
  ...
  return (
    <div>
      <h2 style={{ marginTop: 0, fontWeight: 'bold' }}>Signal History Report</h2>
          ...
          <table className="table table-hover" style={{ fontSize: '13px' }}>
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={5} className="text-center" style={{ padding: '40px', color: '#999' }}>No signal history records found.</td></tr>
              ) : (
                history.map((h: any) => (
                  <tr key={h.id}>
                    ...
                    <td>{h.olt_name}</td>
                    <td style={{ fontWeight: 'bold', color: h.signal < -27 ? 'red' : 'green' }}>{h.signal} dBm</td>
                  </tr>
                ))
              )}
            </tbody>
