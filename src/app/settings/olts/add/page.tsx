...
export default function AddOltPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    olt_user: '',
    olt_password: '',
    snmp_ro_community: 'hbqct.2fYg9L',
    snmp_rw_community: 'p8qfOamnoQ~5',
    snmp_port: '2161',
    iptv: false,
    manufacturer: 'huawei',
    olt_hardware_version: 'Huawei-EA5800-X15',
    olt_version: 'R018',
  ...
  useEffect(() => {
    // Find generation for current hardware
  ...
  const handleTestConnection = async (e: any) => {
    ...
    try {
      const res = await fetch('/api/settings/olt/test', {
        ...
        body: JSON.stringify({
           test_only: true, // Tell backend not to look up ID
           ip: formData.olt_ip,
  ...
  return (
    <div className="container-fluid content-wrap">
      ...
      <form className="form-horizontal" onSubmit={handleSave} acceptCharset="utf-8">
        ...
        <div className="form-group" id="olt_version_group" style={{ display: 'none' }}>
          <label className="control-label col-sm-2" htmlFor="olt_version">OLT software version</label>
          <div className="col-sm-8">
            <select id="olt_version" className="form-control" value={formData.olt_version} onChange={e => setFormData({...formData, olt_version: e.target.value})}>
              {softwareOptions.map(sw => (
                 <option key={sw.id} value={sw.name}>{sw.display_name}</option>
              ))}
            </select>
          </div>
        </div>
        ...
        <div className="form-actions">
          ...
          <div className="col-sm-8">
            ...
            <Link href="/settings/olts" className="btn btn-link">Cancel</Link>
